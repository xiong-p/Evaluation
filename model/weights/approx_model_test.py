    ...
    def _save(self, checkpoint_step, best=False):
        ...
        if best:
            torch.save(
                ...
                f'{os.path.join(self._log_dir, "state")}_best.pt'
            )
...
def main(args):
    ...
    if log_dir is None:
        log_dir = f'./logs/gen.lr:{learning_rate}.batch_size:{batch_size}.dropout:{dropout}.hidden:'+"_".join([str(i) for i in num_hidden_list])
    else:
        if log_dir[-1] != "/":
            log_dir += "/"
        log_dir = log_dir + f'gen.lr:{learning_rate}.batch_size:{batch_size}.dropout:{dropout}.hidden:'+"_".join([str(i) for i in num_hidden_list])
        # log_dir = log_dir + f'gen.lr:{learning_rate}.batch_size:{batch_size}.hidden:'+"_".join([str(i) for i in num_hidden_list])
...
if __name__ == "__main__":
    ...
    #                     help="the probability of the dropout, default 0.0")
